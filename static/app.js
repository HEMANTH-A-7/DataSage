...
async function checkHealth() {
  try {
    ...
    if (dot) dot.style.background = res.ok ? '#0ca678' : '#FF000F';
  } catch { /* silent */ }
}
checkHealth();
...
function renderEDA(data) {
  ...
  qBadge.style.borderColor = qs >= 80 ? 'var(--green)' : qs >= 60 ? 'var(--amber)' : 'var(--abb-red)';
  qBadge.style.color = qs >= 80 ? 'var(--green)' : qs >= 60 ? 'var(--amber)' : 'var(--abb-red)';
...
function renderArena(data) {
  ...
  if (leaderboard.length > 0) {
    ...
    const colors = leaderboard.map((e, i) => i === leaderboard.length - 1 ? '#FF000F' : i === leaderboard.length - 2 ? '#3b82f6' : '#9e9e9e').reverse();
